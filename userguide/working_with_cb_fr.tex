\chapter{Travailler avec \codeblocks}

Ce chapitre traite de quelques connaissances de base pour pouvoir travailler avec \codeblocks. Quelques paragraphes, ici directement repris du Wiki font double emploi mais avec une présentation un peu différente de ce qu'il y a dans le premier chapitre.

\begin{BUILDPROCESS}
\input{build_process_fr}
\end{BUILDPROCESS}

\begin{CREATEPROJECT}
\input{create_project_fr}
\end{CREATEPROJECT}

\begin{DEBUGGING}
\input{debug_project_fr}
\end{DEBUGGING}

\begin{DEBUGGERSCRIPTS}
\input{debugger_scripts_fr}
\end{DEBUGGERSCRIPTS}

\begin{MAKEFILES}
\input{cb_and_makefiles_fr}
\end{MAKEFILES}

\begin{CBP2MAKE}
\input{cbp2make_fr}
\end{CBP2MAKE}

\begin{INTERNATIONALIZATION}
\input{internationalization_fr}
\end{INTERNATIONALIZATION}

\begin{ADDINGLANGUAGES}
\input{adding_language_fr}
\end{ADDINGLANGUAGES}

\begin{VARIABLESTYPES}
\input{variables_types_fr}
\end{VARIABLESTYPES}

\begin{FILEFORMAT}
\input{file_format_desc_fr}
\end{FILEFORMAT}

\begin{DYNAMIC_STATIC}
\input{dynamic_static_linking_fr}
\end{DYNAMIC_STATIC}
