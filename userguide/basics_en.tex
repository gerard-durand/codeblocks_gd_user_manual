...
\textbf{\textit{What is an IDE?}}\\
\codeblocks is an \textbf{IDE}. What does that mean?\\
IDE is for \textbf{I}ntegrated \textbf{D}evelopment \textbf{E}nvironment. It is a graphical environment framework for working with source code and using compilers and linkers (in the case of \codeblocks, these can be several). For several languages, \codeblocks has a built-in syntax highlighting system and offers code completion which helps you to identify language-specific features as keywords, classes, functions, etc.\\
With \codeblocks you'll be able to write codes in different high level languages as C, C++, fortran and some others. \codeblocks will help you to translate these source codes to binary executables. These translations are made with a compiler and a linker. \codeblocks itself, is \textbf{NOT} a compiler nor a linker, but it creates a graphic interface to these tools (and many others) to help you to build your executables by using \textbf{external} compilers and linkers. \codeblocks can use different compilers (and linkers), from Microsoft/Windows world as Visual C, C++, etc, but also from Unix/Linux world as many distributions of CC/C++/gcc/g++. Many compilers/linkers commonly used in Linux have been ported to Windows.\\
If you use Linux, you have already compilers/linkers installed on your system.\\
On Windows, it's generally not the case: no compiler/linker installed. For your convenience, a Windows version of \codeblocks is distributed with a compilation toolchain, MinGW/GCC in 64 or 32 bits, coming from the Linux world. However, this is provided "as-is" and not developed/maintained by the \codeblocks development team. You are not obliged to use this toolchain. Other Windows versions are distributed without a compiler but you'll have to install at least one to be able to build executables.\\
\codeblocks is able to identify by itself some compilers/linkers during installation. In any case, it will be possible, afterwards, to configure \codeblocks to use them or to modify the configuration.\\
It's also possible, in \codeblocks, to support languages that are not natively supported (for example D, java, fortran, rust,…) through the use of plugins that you can find inside \codeblocks distributions, built by users on Internet (on \codeblocks forums for example) or simply by configuring a new language by yourself (of course needs some knowledge and work by you).
\newpage
...
If you wish to define standard settings for several users of a computer, the configuration file \file{default.conf} has to be stored in the directory \file{\osp Documents and Settings\osp Default User\osp Application Data\osp codeblocks} in Win 7, \file{\osp Users\osp Default\osp AppData\osp Roaming\osp CodeBlocks} in Windows 10 or 11, or an equivallent profile path for other OS. During the first startup, \codeblocks will copy the presettings from \samp{Default User} to the application data of the current users.
...
\codeblocks offeres several ways of searching within a file or directory. The dialog box for searching is opened via \menu{Search,Find} (Ctrl-F) or \menu{Find in Files} (Ctrl-Shift-F).

Alt-G and Ctrl-Alt-G are another useful functions. The dialog which will open on using this shortcut, lets you select files/functions and then jumps to the implementation of the selected function (see \pxref{fig:select_function}) or opens the selected file in the editor. You may use wildcards like \codeline{*} or \codeline{?} etc. for an incremental search in the dialog.
...
\subsection{Switching between projects or files}
...
\subsection{Displaying several files on the same screen}

As seen above, F11 is usefull to switch between a file \file{\var{name}.cpp} and the corresponding header file \file{\var{name}.h}: They use the same editor view. An other usefull option is to split only one file on the screen in two parts, horizontally or vertically. This is obtained by a right click on the tab containing the name of the desired file, and choosing the right option in the split view sub-menu. Then you can navigate independantly in each part of the file.

Nevertheless, it's also usefull to split the display window into two (or more) parts and display in each sub-view a different file (for example a source file \file{\var{name}.cpp} in one view and the corresponding header file \file{\var{name}.h} in the other view. A way to achieve this, is to have a project with several editor tabs displayed (one file in each tab). With your mouse, you select then drag and release one tab at the top (or the bottom) of the editor window to split horizontally, or completely at the right (or the left) of the editor window to split vertically. A blue "ghost" windows appears in the editor during the mouse drag. The second view is created after the mouse release.

\figures[H][width=\columnwidth]{splitting_horizontally}{Drag and release to obtain a horizontal splitted view with two different files}
\figures[H][width=\columnwidth]{splitting_vertically}{Drag and release to obtain a vertical splitted view with two different files}

\hint{You can do this several times, but on too small screens you will obtain too many subwindows, not easy to read}
...
In \codeblocks, you can choose between several ways of treating file extensions. The settings dialog can be opened via \menu{Settings,Files extension handling}.
...
\iftrue    % with iftrue displays as an array, with iffalse displays as a description
\newpage
...
\textbf{/h, /?} & \footnotesize{\opt{--help, --?}}              & Shows a help message regarding the command line arguments.                        \\ \hline
                & \footnotesize{\opt{--safe-mode}}              & Load in safe mode (All plugins are disabled on startup.)           				\\ \hline
...
\textbf{/v}     & \footnotesize{\opt{--verbose}}                & Show more debugging c::b messages.                                                \\ \hline
...
\textbf{/S} name& \footnotesize{\opt{--set=name}}               & Set "name" to current active set for global variables (after [r13245])         	\\ \hline
\textbf{/D} str & \footnotesize{\opt{-D $<$set$>$.uservar.mem=val}} & Set the member mem of user variable uservar in "set" to value val. This
...
\end{samepage}
\begin{description}
\item[\opt{--}file=\var{filename}\optional{:line}] Open file in \codeblocks and optionally jump to a specific line.
...
\item[\opt{--}prefix=\var{str}] Sets the shared data directory prefix.
\item[\opt{--}user-data-dir=\var{str}] Specify an alternative directory for user settings and user installed plugins. 
\item[/p, \opt{--}personality=\var{str}, \opt{--}profile=\var{str}] Sets the personality to use. You can use ask as the parameter to list all available personalities.
...
\item[\opt{--}target=\var{str}] Sets target for batch build. For example \cmdline{\opt{--}target='Release'}.
...
\item[\opt{--}script=\var{str}] Execute script file.
\item[\opt{--}dbg-config=\var{str}] Selects the debugger config used for attaching (uses the current selected target if omitted).
\item[\opt{--}dbg-attach=\var{str}] String passed to the debugger plugin which is used for attaching to a process.
