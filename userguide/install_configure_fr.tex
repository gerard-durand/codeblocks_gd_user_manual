...
\item Le fichier codeblocks-25.03mingw-setup.exe inclut en plus un compilateur GCC/G++ et un débugueur GDB provenant d'une version MinGW64 14.2, 64 bits. Ce fichier inclut un compilateur Gfortran.
\item Le fichier codeblocks-25.03mingw-nosetup.zip est fourni par commodité aux utilisateurs qui sont allergiques aux installateurs. Toutefois, vous ne pourrez pas choisir les extensions / fonctionnalités à installer (tout est inclus) ni même créer les raccourcis de menu. Pour l'"installation" vous êtes seuls.
