...
\hint{25.03 version does not still exist. Sorry. Read 20.03 instead, but a new version is scheduled for release in early 2025, so a 25.xx, with xx as the number of the publication month. Text below will be updated as soon as possible.}
Install steps:
\begin{itemize}
...
\item The codeblocks-25.03mingw-setup.exe file includes additionally the GCC/G++ compiler and GDB debugger version MinGW64 14.2, 64 bit. The codeblocks-25.03mingw\_fortran-setup.exe file includes additionally to that the Gfortran compiler.
